...
void CClientSocket::updateEvent(CString showMsg,CString from,bool reset,int timeFMT){
	...
	switch (timeFMT)
	{
	case 1:sysMsg = from + "  " + time.Format(_T("%Y/%m/%d %H:%M:%S  "));break;
	case 2:sysMsg = from + "  " + time.Format(_T("%H:%M:%S  "));break;
	case 3:sysMsg = from=="" ? from : from+"  ";
	default:
		break;
	}
	if(str!="" && reset){
...
void CClientSocket::OnReceive(int nErrorCode)
{
	...
	{
		sprintf_s(pDlg->nd.szTip, "客户端：%s - 离线",userID);
	...
	elif(msg.userId==userID) //给自己的消息
	{
		if (pDlg->logining){	//正在登陆则把前一次的连接断开
			...
			elif(msg.type == TYPE[UserList])	//收到了用户列表，则表示登录成功
			{
				...
				sprintf_s(pDlg->nd.szTip, "客户端：%s - 在线", userID);	//状态在登陆判重时用到
				Shell_NotifyIcon(NIM_MODIFY, &pDlg->nd);
				pDlg->m_connect = true;
				...
				if (!strstr(pDlg->nd.szTip, "在线")){			//拒绝正在登陆的用户
					pDlg->KillTimer(0);
			...
			elif(msg.type == TYPE[OfflineMsg]){	//离线消息结束
				static MyMsg olmsg;
			...
			elif(msg.type == TYPE[Status]){	//离线消息结束
...
void CClientSocket::SendMSG(CString send,bool upEvent)
{
	...
	{
		CString str;
		str.Format("（错误代码：%d）", getError());
		if(tmp_msg.type!=TYPE[I_am_online]){
			MessageBox(hWnd, "发送失败" + str, "温馨提示",0);
		}else{
			(CmfcClient1Dlg*)theApp.GetMainWnd()->KillTimer(1);
			sprintf_s(pDlg->nd.szTip, "客户端：%s - 离线",userID);
			Shell_NotifyIcon(NIM_MODIFY, &pDlg->nd);
			if (pDlg->m_connect)      // 如果已经连接，则断开服务器
			{
			...
			pDlg->firstCon = 1;
			pDlg->SetTimer(4,2000,0);
			MessageBox(hWnd, "与服务器连接异常，请尝试重连" + str, "温馨提示",0);
		}
		return ;
	}
	if(tmp_msg.type!=TYPE[I_am_online])
